...
export default async function handler(req,res){
    if(req.method === "POST"){
        const {noWa,nik,nama,tanggal_lahir,tempat_lahir,status,anak_ke,jumlah_saudara,pendidikan,nama_ayah,nama_ibu,kondisi,tempat_lahir_ayah,tanggal_lahir_ayah,jenis_kelamin_ayah,umur_ayah,hari_meninggal_ayah,tanggal_meninggal_ayah,meninggal_karena_ayah,tempat_lahir_ibu,tanggal_lahir_ibu,jenis_kelamin_ibu,umur_ibu,hari_meninggal_ibu,tanggal_meninggal_ibu,meninggal_karena_ibu} = req.body
        ...
        if (status == 'Yatim'){
            ...
            const adddata = await prisma.data_pengajuan.create({
                data:{
                    noWa,
                    nik,
                    nama,
            ...
            return res.status(200).json({message:"Data Berhasil Ditambahkan Untuk File akan Dikirim Segera Melalui Whatsapp!"})
        }
        if (status == 'Piatu'){
        ...
        if (status == 'Yatim-Piatu'){
            ...
            if(req.method === "GET"){
                const data = await prisma.data_pengajuan.findMany({
                    orderBy:{
                        id:'desc'
                    }
                })
